...
const byte numDevices = 1;      // number of MAX7219s used
// inputs: DIN pin, CLK pin, LOAD pin. number of chips
#define blue 7// data in
#define green 6// clock pin
#define yellow 5//cs pin 
...
void loop() {
     ...
     */
     lc.setColumn(0, 0, bar(av(fht_log_out, 33, 40) * 2));
        lc.setColumn(0, 1, bar(av(fht_log_out, 41, 46) * 2));
        lc.setColumn(0, 2, bar(av(fht_log_out, 47, 52) * 2));
        lc.setColumn(0, 3, bar(av(fht_log_out, 53, 58) * 2));
        lc.setColumn(0, 4, bar(av(fht_log_out, 59, 64) * 2));
        lc.setColumn(0, 5, bar(av(fht_log_out, 65, 70) * 2));
        lc.setColumn(0, 6, bar(av(fht_log_out, 71, 76) * 2));
        lc.setColumn(0, 7, bar(av(fht_log_out, 77, 82) * 2));   
  } 
}
